...
program dckernel_divdamp
  ...
  open( unit   = EX_fid,         &
     ...
     read(EX_fid) ORG_ddivdx           (:,:,:)
     read(EX_fid) ORG_ddivdx_pl        (:,:,:)
     read(EX_fid) ORG_ddivdy           (:,:,:)
     read(EX_fid) ORG_ddivdy_pl        (:,:,:)
     read(EX_fid) ORG_ddivdz           (:,:,:)
     read(EX_fid) ORG_ddivdz_pl        (:,:,:)
     read(EX_fid) ORG_rhogvx           (:,:,:)
     read(EX_fid) ORG_rhogvx_pl        (:,:,:)
     read(EX_fid) ORG_rhogvy           (:,:,:)
     read(EX_fid) ORG_rhogvy_pl        (:,:,:)
     read(EX_fid) ORG_rhogvz           (:,:,:)
     read(EX_fid) ORG_rhogvz_pl        (:,:,:)
     ...
     read(EX_fid) ORG_VMTR_RGSQRTH     (:,:,:)
     read(EX_fid) ORG_VMTR_RGSQRTH_pl  (:,:,:)
     read(EX_fid) ORG_VMTR_C2WfactGz   (:,:,:,:)
     read(EX_fid) ORG_VMTR_C2WfactGz_pl(:,:,:,:)
