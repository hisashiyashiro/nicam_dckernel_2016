...
program dckernel_vi_rhow_solver
  ...
  open( unit   = EX_fid,         &
     ...
     read(EX_fid) ORG_rhogw_prev      (:,:,:)
     read(EX_fid) ORG_rhogw_prev_pl   (:,:,:)
     read(EX_fid) ORG_rhogw           (:,:,:)
     read(EX_fid) ORG_rhogw_pl        (:,:,:)
     read(EX_fid) ORG_rhogw0          (:,:,:)
     read(EX_fid) ORG_rhogw0_pl       (:,:,:)
     read(EX_fid) ORG_preg0           (:,:,:)
     read(EX_fid) ORG_preg0_pl        (:,:,:)
     read(EX_fid) ORG_rhog0           (:,:,:)
     read(EX_fid) ORG_rhog0_pl        (:,:,:)
     read(EX_fid) ORG_Srho            (:,:,:)
     read(EX_fid) ORG_Srho_pl         (:,:,:)
     read(EX_fid) ORG_Sw              (:,:,:)
     read(EX_fid) ORG_Sw_pl           (:,:,:)
     read(EX_fid) ORG_Spre            (:,:,:)
     read(EX_fid) ORG_Spre_pl         (:,:,:)
     read(EX_fid) ORG_Mc              (:,:,:)
     read(EX_fid) ORG_Mc_pl           (:,:,:)
     read(EX_fid) ORG_Ml              (:,:,:)
     read(EX_fid) ORG_Ml_pl           (:,:,:)
     read(EX_fid) ORG_Mu              (:,:,:)
     read(EX_fid) ORG_Mu_pl           (:,:,:)
     read(EX_fid) ORG_VMTR_RGSGAM2    (:,:,:)
     read(EX_fid) ORG_VMTR_RGSGAM2_pl (:,:,:)
     read(EX_fid) ORG_VMTR_RGSGAM2H   (:,:,:)
     read(EX_fid) ORG_VMTR_RGSGAM2H_pl(:,:,:)
     read(EX_fid) ORG_VMTR_RGAMH      (:,:,:)
     read(EX_fid) ORG_VMTR_RGAMH_pl   (:,:,:)
     read(EX_fid) ORG_VMTR_RGAM       (:,:,:)
     read(EX_fid) ORG_VMTR_RGAM_pl    (:,:,:)
     read(EX_fid) ORG_VMTR_GSGAM2H    (:,:,:)
     read(EX_fid) ORG_VMTR_GSGAM2H_pl (:,:,:)
  close(EX_fid)
